...
def upgrade() -> None:
    ...
    session.add_all(
        [
            StatusEntity(code=status_enum.value, name=status_enum.value)
    ...
    session.add_all(
        [
            TypeEntity(code=type_enum.value, name=type_enum.value)
...
def downgrade() -> None:
    ...
    delete_status_entity_sql = delete(StatusEntity).where(
        StatusEntity.code.in_([status_enum.value for status_enum in StatusEnum])
    )
    session.execute(delete_status_entity_sql)

    delete_type_entity_sql = delete(TypeEntity).where(
        TypeEntity.code.in_([type_enum.value for type_enum in TypeEnum])
    )
