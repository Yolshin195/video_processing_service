...
class TaskEntity(BaseEntity):
    ...
    status: Mapped["StatusEntity"] = relationship(foreign_keys=status_id)

    type_id: Mapped[UUID] = mapped_column(ForeignKey("type_entity.id"))
    type: Mapped["TypeEntity"] = relationship(foreign_keys=type_id)

    input_file_id: Mapped[UUID | None] = mapped_column(ForeignKey("file_entity.id"))
    input_file: Mapped["FileEntity"] = relationship(foreign_keys=input_file_id)

    output_file_id: Mapped[UUID | None] = mapped_column(ForeignKey("file_entity.id"))
    output_file: Mapped["FileEntity"] = relationship(foreign_keys=output_file_id)
