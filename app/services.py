...
from .models import TaskModel, HandleYoutubeVideo, HandleTorrentFileVideo, HandleCustomUserVideo
...
class TaskService(BaseService):
    ...
    async def find_all(self, skip: int = 0, limit: int = 100) -> list[TaskModel]:
        return [task_mapper(entity) for entity in await self.repository.find_all(skip, limit)]
    ...
    async def handle_youtube_video(self, event: HandleYoutubeVideo) -> TaskModel:
        current_status = await self.status_repository.find_by_code(StatusEnum.CREATED.value)
